...
release version:
	...
	# main.js), so it is not a separate release asset. The CI workflow
	# release-binaries.yml attaches the per-platform node-pty zips and
	# checksums.json after this release is published.
